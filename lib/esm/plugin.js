...
export const updateMediaProfile = (plugin, profile, profileRepo) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (existing) {
        const parsed = yield parseProfile(profileRepo, profile, plugin);
        const update = Object.assign({}, existing, parsed);
        profile = yield profileRepo.update(update);
    }
    else {
        const parsed = yield parseProfile(profileRepo, profile, plugin);
        profile = yield profileRepo.create(parsed);
    }
    const finalizedProfile = yield parseProfile(profileRepo, profile, plugin);
...
const parseProfile = (profileRepo, profile, plugin) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (prof.additionalAssets && prof.additionalAssets.length > 0) {
        parsed.additionalAssetsRegexes = prof.additionalAssets.map((re) => new RegExp(re));
    }
    if (!parsed.noop) {
...
export const applyProfile = (logger, downloaded, media, profileName, outDir, sourcePath, conn, analysisResults) => __awaiter(void 0, void 0, void 0, function* () {
