...
export const parseProfile = (mediaProfileRepo, profile) => __awaiter(void 0, void 0, void 0, function* () {
    if (!PARSED_PROFILES[profile.name]) {
        let fromProfile = null;
        if (profile.from) {
            const fromProfileObj = yield mediaProfileRepo.findById(profile.from);
            fromProfile = yield parseProfile(mediaProfileRepo, fromProfileObj);
        }
        ...
        if (profile.subProfiles && profile.subProfiles.length > 0) {
            ...
            for (const subProf of profile.subProfiles) {
                ...
                parsed.subProfileObjects.push(yield parseProfile(mediaProfileRepo, subProfObject));
            }
        }
